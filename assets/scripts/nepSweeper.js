...
class NepSweeper {
  ...
  setDifficulty = (difficultyName) => {
    ...
    const newDifficulty = this.difficulties.find(
  ...
  setSpriteSheet = (spriteSheetName) => {
    ...
    if (newSpriteSheet) {
      ...
      root.style.setProperty(
        ...
        'url("../images/minesweeper/' + closestSprite + '")'
      );
      root.style.setProperty(
        "--nepSweeper-sprite-sheet-color",
        newSpriteSheet.color
      );
  ...
  getRandomEmptyIndex = (skipIndices = new Array()) => {
    ...
    if (skipIndices.length) {
      emptyIndices = emptyIndices.filter(
        (index) => !skipIndices.includes(index)
      );
    }
  ...
  openLeaderBoard = () => {
    ...
    leaderBoardScoresWrapper.classList.add(
      "nepSweeper-leaderBoardScoresWrapper"
    );

    this.difficulties.forEach((difficultySetting) => {
      const difficultyWrapper = document.createElement("div");
      difficultyWrapper.classList.add(
        "nepSweeper-leaderBoardDifficultyWrapper"
      );
      ...
      if (Array.isArray(difficultyScores)) {
        ...
        difficultyScores.forEach((score) => {
          ...
          scoreElement.innerHTML =
            score.time + "\xa0\xa0 | " + scoreTime.toLocaleString();
  ...
  closeLeaderBoard = () => {
    const leaderBoardWrapper = document.getElementsByClassName(
      "nepSweeper-leaderBoardWrapper"
    )[0];
  ...
  showLeaderBoardDifficulty = (difficultyName) => {
    const leaderBoardScoresWrapper = document.querySelector(
      ".nepSweeper-leaderBoardScoresWrapper"
    );
    ...
    leaderBoardScores.forEach((score) => {
      ...
      const scoreElement = document.createElement("li");
      scoreElement.innerHTML =
        score.time + " (" + scoreTime.toLocaleString() + ")";
  ...
  firstPrint = () => {
    if (!document.getElementById("nepSweeperStyle")) {
      ...
      nepSweeperStyle.href = "./assets/styling/nepSweeperStyle.css?v=2.817";
    ...
    const gameWrapper = document.getElementsByClassName(
      "nepSweeper-gameWrapper"
    )[0];
    ...
    gameContainer.style.height =
      (this.yLength * 16 + 62) * this.currentSpriteSheetZoomScale + "px";
    gameContainer.style.width =
      (this.xLength * 16 + 20) * this.currentSpriteSheetZoomScale + "px";
    gameWrapper.appendChild(gameContainer);

    // Top menu border
    gameContainer.appendChild(
      this.createGameSpriteElement("topLeftCornerBorder")
    );
    for (let i = 0; i < this.xLength; i++) {
      gameContainer.appendChild(
        this.createGameSpriteElement("horizontalBorder")
      );
    }
    gameContainer.appendChild(
      this.createGameSpriteElement("topRightCornerBorder")
    );
    // Menu
    gameContainer.appendChild(
      this.createGameSpriteElement("longVerticalBorder")
    );
    const mineContainer = document.createElement("div");
    mineContainer.classList.add(
      `mineContainer${this.currentSpriteSheetZoomScale}`
    );
    ...
    timeContainer.classList.add(
      `timeContainer${this.currentSpriteSheetZoomScale}`
    );
    ...
    // Bottom menu border
    gameContainer.appendChild(
      this.createGameSpriteElement("rightConnectionBorder")
    );
    for (let i = 0; i < this.xLength; i++) {
      gameContainer.appendChild(
        this.createGameSpriteElement("horizontalBorder")
    ...
    gameContainer.appendChild(
      this.createGameSpriteElement("leftConnectionBorder")
    );
    // Game board
    ...
    // Bottom border
    gameContainer.appendChild(
      this.createGameSpriteElement("bottomLeftCornerBorder")
    );
    for (let i = 0; i < this.xLength; i++) {
      gameContainer.appendChild(
        this.createGameSpriteElement("horizontalBorder")
    ...
    gameContainer.appendChild(
      this.createGameSpriteElement("bottomRightCornerBorder")
    );
  };
  ...
  addPressedState = (tileIndex) => {
    if (!this.gameLost) {
      if (this.leftPressed) {
        document
          .querySelector(`[tile="${tileIndex}"]`)
          .classList.add("pressed");
      }
      if (this.leftPressed && this.rightPressed) {
        const surroundingIndices = this.getsurroundingIndices(
          this.mouseHoverTile
        );
        surroundingIndices.forEach((index) => {
  ...
  createGameSpriteElement = (className) => {
    ...
    spriteElement.classList.add(
      `gameSprite${this.currentSpriteSheetZoomScale}`,
      className
    );
  ...
  printTime = () => {
    const timeNumbers = document.querySelector(
      `.timeContainer${this.currentSpriteSheetZoomScale}`
    ).children;
    if (this.firstClickTime) {
      ...
      if (this.secondsSinceStart < 999) {
        timeNumbers[0].className =
          timeNumbers[0].className.slice(0, -1) +
          Math.floor((this.secondsSinceStart / 10 / 10) % 10);
        timeNumbers[1].className =
          timeNumbers[0].className.slice(0, -1) +
          Math.floor((this.secondsSinceStart / 10) % 10);
        timeNumbers[2].className =
          timeNumbers[0].className.slice(0, -1) +
          Math.floor(this.secondsSinceStart % 10);
      } else {
  ...
  printMinesLeft = () => {
    const mineNumbers = document.querySelector(
      `.mineContainer${this.currentSpriteSheetZoomScale}`
    ).children;
    if (this.minesLeft >= 0) {
      mineNumbers[0].className =
        mineNumbers[0].className.slice(0, -1) +
        Math.floor((this.minesLeft / 10 / 10) % 10);
      mineNumbers[1].className =
        mineNumbers[0].className.slice(0, -1) +
        Math.floor((this.minesLeft / 10) % 10);
      mineNumbers[2].className =
        mineNumbers[0].className.slice(0, -1) + Math.floor(this.minesLeft % 10);
    } else {
      mineNumbers[0].className = mineNumbers[0].className.slice(0, -1) + "-";
      const positiveMinesLeft = this.minesLeft * -1;
      mineNumbers[1].className =
        mineNumbers[0].className.slice(0, -1) +
        Math.floor((positiveMinesLeft / 10) % 10);
      mineNumbers[2].className =
        mineNumbers[0].className.slice(0, -1) +
        Math.floor(positiveMinesLeft % 10);
    }
  };
...
window.addEventListener("keydown", (e) => {
  ...
  if (
    userSecret.toLowerCase() === secretString &&
    !document.getElementsByClassName("nepSweeper-secretContainer")[0]
  ) {
